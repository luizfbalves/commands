...
**FUNDAMENTAL RULE:** The entire system uses the cents pattern to store and transmit monetary values (API, Database, WebSocket). Conversion to reais (decimals) happens **ONLY** in the frontend for display purposes.

- **API/Backend:** Sends/receives `price: 3590` (representing R$ 35.90).
- **Database:** Stores `3590` (INTEGER).
- **Frontend:** Receives `3590`, uses utilities like `centsToReais` or `<FormattedPrice>` to display "R$ 35.90".
...
- **[ ] Calculations with Decimals:** Are mathematical operations (`+`, `-`, `*`, `/`) being performed with values in reais (decimals) instead of cents? This violates the precision principle.
...
- **[ ] Form Input Handling:** Is a price input's value being sent to the API as a string or decimal without the `reaisToCents` conversion?
...
- **[ ] Incorrect Utility Usage:** Is the code using `centsToReais` to send data to the API (error) or trying to format a value that is already in reais (logical error)?
...
1.  **Identify the Problem and Location:** "Problem: Total calculation with reais values in file `cart-summary.tsx`, line 15."
...
When proposing a fix for value display or conversion, you **MUST** use the functions from `apps/web/lib/utils/price.ts`. Do not implement manual formatting (e.g., `(price / 100).toFixed(2)`). Instead, use the provided utility functions like `formatPrice` or `centsToReais` from that specific file.
